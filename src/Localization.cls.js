...
Localization.prototype = {
     ...
     * @type {string}
     */
    userLanguage    : inBrowser ? navigator.language 
                                : process.env.LANG || 
...
function Localization ()
{
        ...
        if ( strings != null && strings !== '' )
        {
            ...
            if( reg )
            {
                for ( var i=0 ,l=reg.length ; i<l ; i++ )
                {
                    ...
                    if ( (/^[\s|\t]*\[.+\];$/).test(value) )// Is it a multi-lines value ?
                    {
                    ...
                    Localization.strings[key.replace(/^"(.+)"[\s|\t]*$/,'$1')] = value;
                }
                return true;
            }
        }
    ...
    var fs;
    var loadFile = function loadFile () { return '' };
    if ( inBrowser )
    {
        ...
        {    
            ...
            xreq.onerror = function() { console.log('loading failed on ' + file) }
...
if (typeof define === "function" && define.amd) {
    define([], function() { return Localization })
